...
#ifdef HILOG
#include "hilog/log.h"
#define SRV_LOG(fmt, ...) OH_LOG_Print(LOG_APP, ::LOG_INFO, 0x0000, "HDC_SRV", fmt, ##__VA_ARGS__)
#else
#define SRV_LOG(fmt, ...)
#endif
...
{
    bool ret = false;
    // ProgramMutex skipped - coexist with system hdcd, we just need UDS socket
    SRV_LOG("Initial entry, listenString=%{public}s", listenString ? listenString : "(null)");
    Base::RemoveLogFile();
    do {
        SRV_LOG("Initial: creating HdcServerForClient");
        clsServerForClient = new HdcServerForClient(true, listenString, this, &loopMain);
        SRV_LOG("Initial: calling HdcServerForClient::Initial");
        int rc = (static_cast<HdcServerForClient *>(clsServerForClient))->Initial();
        SRV_LOG("Initial: HdcServerForClient::Initial rc=%{public}d", rc);
        if (rc != RET_SUCCESS) {
            WRITE_LOG(LOG_FATAL, "clsServerForClient Initial failed");
            break;
        }
        SRV_LOG("Initial: creating HdcHostTCP");
        clsTCPClt = new HdcHostTCP(true, this);
        SRV_LOG("Initial: creating HdcHostUSB");
        clsUSBClt = new HdcHostUSB(true, this, ctxUSB);
        if (clsUSBClt) { clsUSBClt->InitLogging(ctxUSB); }
        int usbInitRc = clsUSBClt->Initial();
        SRV_LOG("Initial: USB_Initial rc=%{public}d", usbInitRc);
        if (usbInitRc != RET_SUCCESS) {
...
#endif
        SRV_LOG("Initial: all init passed, starting event loop");
        Base::ProcessCmdLogs();
        ret = true;
    } while (0);
    SRV_LOG("Initial returning ret=%{public}d", ret);
    if (!ret) {
