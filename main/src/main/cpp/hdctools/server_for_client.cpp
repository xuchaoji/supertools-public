...
#ifdef HILOG
#include "hilog/log.h"
#define UDS_LOG(fmt, ...) OH_LOG_Print(LOG_APP, ::LOG_INFO, 0x0000, "HDC_UDS", fmt, ##__VA_ARGS__)
#else
#define UDS_LOG(fmt, ...)
#endif
#ifndef TEST_HASH
...
bool HdcServerForClient::SetUdsListen()
{
    ...
    UDS_LOG("uv_pipe_init ret=%{public}d", ret);
    WRITE_LOG(LOG_WARN, "SetUdsListen uv_pipe_init ret=%d", ret);

    unlink(UDS_PATH.c_str());
    UDS_LOG("unlink done, binding to %{public}s", UDS_PATH.c_str());

    ret = uv_pipe_bind(&udsListen, UDS_PATH.c_str());
    if (ret != 0) {
        UDS_LOG("bind FAILED ret=%{public}d errno=%{public}d path=%{public}s", ret, errno, UDS_PATH.c_str());
        WRITE_LOG(LOG_WARN, "bind uds addr %s fail! ret:%d errno:%d", UDS_PATH.c_str(), ret, errno);
        return false;
    }
    UDS_LOG("bind success");
    WRITE_LOG(LOG_WARN, "SetUdsListen bind success");

    uv_fs_t req = {};
    ret = uv_fs_chmod(nullptr, &req, UDS_PATH.c_str(), S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, nullptr);
    if (ret < 0) {
        char buffer[BUF_SIZE_DEFAULT] = { 0 };
        uv_strerror_r(ret, buffer, BUF_SIZE_DEFAULT);
        UDS_LOG("chmod failed: %{public}s (non-fatal)", buffer);
        WRITE_LOG(LOG_WARN, "uv_fs_chmod uds failed %s (non-fatal, continuing)", buffer);
    }
    uv_fs_req_cleanup(&req);
    UDS_LOG("chmod done, calling uv_listen");

    if ((ret = uv_listen((uv_stream_t *)&udsListen, MAX_CONNECTIONS_COUNT, AcceptUdsClient)) != 0) {
        UDS_LOG("uv_listen FAILED ret=%{public}d", ret);
        WRITE_LOG(LOG_WARN, "uds listen fail! ret:%d", ret);
        return false;
    }
    UDS_LOG("SetUdsListen success");
    WRITE_LOG(LOG_WARN, "SetUdsListen success");
    return true;
}
#endif
...
#else
    UDS_LOG("Initial: __OHOS__ branch, hostPort=%{public}s host=%{public}s port=%{public}d",
        channelHostPort.c_str(), channelHost.c_str(), channelPort);
    if (!channelHostPort.size() || !channelHost.size() || !channelPort) {
        UDS_LOG("Initial: invalid listen string, trying UDS only");
        WRITE_LOG(LOG_FATAL, "Listen string initial failed");
    } else {
        UDS_LOG("Initial: trying SetTCPListen");
        if (!SetTCPListen()) {
            WRITE_LOG(LOG_FATAL, "SetTCPListen failed");
        }
    }
    UDS_LOG("Initial: calling SetUdsListen");
    if (!SetUdsListen()) {
        UDS_LOG("Initial: SetUdsListen FAILED (non-fatal, TCP fallback)");
        WRITE_LOG(LOG_FATAL, "SetUdsListen failed (non-fatal)");
    }
    UDS_LOG("Initial: SUCCESS");
#endif
